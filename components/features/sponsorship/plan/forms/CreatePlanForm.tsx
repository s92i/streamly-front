...
export function CreatePlanForm() {
  const t = useTranslations("layout.dashboard.plans.createForm");

  const [isOpen, setIsOpen] = useState(false);
  const { refetch } = useFindMySponsorshipPlansQuery();

  const form = useForm<TypeCreatePlanSchema>({
    resolver: zodResolver(createPlanSchema) as unknown as Resolver<
      TypeCreatePlanSchema,
      any
    >,
    defaultValues: { title: "", description: "", price: 1 },
  });
